...
from src.utils import matmul, KnownT, check_quality, SAVED_QUERIES, run, AcceptableFailure
...
def solve_final_layer(known_T, inputs, outputs):
    ...
    extracted_path = os.path.join(temp_dir, "extracted-%s.p" % "-".join(map(str, sizes)))
    real_path = os.path.join(temp_dir, "real-%s.p" % "-".join(map(str, sizes)))

    print("SAVING", extracted_path)
    
    pickle.dump([At,
                 Bt],
                open(extracted_path,"wb"))
