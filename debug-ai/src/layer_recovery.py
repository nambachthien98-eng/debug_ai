...
def graph_solve(all_ratios, all_criticals, expected_neurons, LAYER, debug=False):
    ...
    if len(resulting_rows)+skips_because_of_nan < expected_neurons and len(all_ratios) < DEAD_NEURON_THRESHOLD:
        ...
        raise AcceptableFailure(partial_solution=(np.array(resulting_rows), resulting_examples))
    else:
        ...
        while len(resulting_rows) < expected_neurons:
            if resulting_rows:
                ...
                # We found nothing. The shape of a row is the number of neurons in the previous layer.
                row_shape = (all_ratios.shape[1],)
                resulting_rows.append(np.zeros(row_shape))
                # The shape of an example is the input dimension of the whole network.
                example_shape = (DIM,)
                resulting_examples.append([np.zeros(example_shape)])
