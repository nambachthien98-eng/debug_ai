...
# Convert JAX arrays back to standard NumPy arrays for saving and create a container array
A_final = [onp.array(x) for x in params[0]]
B_final = [onp.array(y) for y in params[1]]
params_to_save = onp.empty(2, dtype=object)
params_to_save[0] = A_final
params_to_save[1] = B_final

# Save our amazing model.
onp.save("models/" + str(seed) + "_" + "-".join(map(str,sizes)), params_to_save)
